...
public class UpdateProductController extends HttpServlet {
    ...
    DAOProduct dao = new DAOProduct();
    Product currentProduct = dao.getProductByID(id);
    ...
    int sellid;
    if (sellidStr != null && !sellidStr.isEmpty()) {
        sellid = Integer.parseInt(sellidStr);
    } else {
        sellid = currentProduct.getSell_ID();
    }
    boolean productUpdated = dao.updateProduct(name, image, price, title, description, cateid, sellid, id);

    if (productUpdated) {
        response.sendRedirect("manage-product");
    } else {
        request.setAttribute("mess", "Failed to Update Product");
        request.getRequestDispatcher("EditProduct.jsp").forward(request, response);
    }
}
